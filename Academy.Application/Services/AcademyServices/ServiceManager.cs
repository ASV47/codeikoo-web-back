...
{
    public class ServiceManager(UserManager<ApplicationUser> userManager, IConfiguration configuration, IUnitOfWork unitOfWork, IMapper mapper, IEmailSender _emailSender) : IServiceManager
    {
        ...
        private readonly Lazy<IJobApplicationService> _LazyJobApplicationService = new Lazy<IJobApplicationService>(() => new JobApplicationService(unitOfWork, mapper));
        private readonly Lazy<IInstructorApplicationService> _LazyInstructorApplicationService = new Lazy<IInstructorApplicationService>(() => new InstructorApplicationService(unitOfWork, mapper));
        private readonly Lazy<ICourseService> _LazyCourseService = new(() => new CourseService(unitOfWork, mapper));
		...
		private readonly Lazy<IArticleService> _LazyArticleService = new Lazy<IArticleService>(() => new ArticleService(unitOfWork, mapper));
		...
		private readonly Lazy<IClientService> _LazyClientService = new Lazy<IClientService>(() => new ClientService(unitOfWork, mapper));
		...
		private readonly Lazy<IFlexibilityItemService> _LazyFlexibilityItemService = new Lazy<IFlexibilityItemService>(() => new FlexibilityItemService(unitOfWork, mapper));
